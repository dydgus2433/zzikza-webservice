...
//		formData.append('content', JSON.stringify(quill.getContents()));
		formData.append('content', quill.container.firstChild.innerHTML);
			...
			$.ajax({
			    type : "POST",
			    url : contextPath + '/api/insertBoard',
			    data : formData,
			    processData: false,
			    contentType: false,
			}).done(function(data) {
...
//	 }
     ...
     $("#fileList").append("<div><p class=\"file_name\"><a href=\"#\">" + filesArr[i].name + "</a><img src=\"/resources/images/common/btn_del.png\" onclick=\"deleteFile(event, " + i + ");\"></p>"+"</div>");
 }
// 
...
function deleteFile (eventParam, orderParam) {
 ...
 for(var i=0; i<filesTempArrLen; i++) {
     innerHtmlTemp += "<div><p class=\"file_name\"><a href=\"#\">" + filesTempArr[i].name + "</a><img src=\"/resources/images/common/btn_del.png\" onclick=\"deleteFile(event, " + i + ");\"></p>"+"</div>"
 }
