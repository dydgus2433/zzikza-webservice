...
function fileUploadSetting(){
	...
	var url = contextPath + '/api/uploadRequestReplFile';  // 사용
	var upload = $('#detailFiles').fileupload({
	    ...
	    add: function(e, data){
	    	...
	    	console.log(e,data)
	    	var id = "img_id_"+data.result.reqReplFlId;
	    	$("#sortable li:last").remove();
			 var html = "<li index=\""+data.result.reqReplFlId+"\" id=\""+id+"\">"	
					+"<div class=\"img_area\" >"
...
function deleteImageAction(e){
	...
	if(src){
		//이거로 파일 삭제해야함
		 ...
		 $.ajax({
			url: contextPath + "/api/deleteRequestReplFile",
...
function indexing(){
	...
	$.ajax({
		url: contextPath + "/api/updateRequestReplFileOrder",
...
function submitAction(){
    ...
    $.ajax({
		url: contextPath+'/api/insertRequestRepl',
		data : formData,
		...
		if(data.rows && data.rows > 0){
			alert('요청상품이 등록되었습니다.');
			location.href = contextPath + '/request/list';
		}else{
